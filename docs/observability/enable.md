...
Each metrics-publishing charm runs an exporter as a sidecar in the same pod as the workload. Charmarr deliberately reuses well-maintained community exporters rather than rolling its own, so the underlying images are the standard ones the upstream communities curate.

| Charm | Exporter |
|---|---|
| radarr, sonarr, prowlarr | [`thecfu/scraparr`](https://github.com/thecfu/scraparr) |
| qbittorrent | [`martabal/qbittorrent-exporter`](https://github.com/martabal/qbittorrent-exporter) |
| sabnzbd | [`msroest/sabnzbd_exporter`](https://github.com/msroest/sabnzbd_exporter) |
| plex | [`axsuul/plex-media-server-exporter`](https://github.com/axsuul/plex-media-server-exporter) |
| gluetun | [`thecfu/gluetun-exporter`](https://github.com/thecfu/gluetun-exporter) |
| flaresolverr | none (no upstream exporter exists) |
| seerr, charmarr-storage, charmarr-crowsnest | none (charm-emitted only) |

Charms without a workload exporter still ship topology metrics directly from the charm container, so they still light up the fleet view in crowsnest.
