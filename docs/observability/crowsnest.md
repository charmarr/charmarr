...
Crowsnest is a workloadless charm. It has no upstream application to babysit. Its only job is to sit above the fleet and watch, which is, of course, the whole point of a crowsnest. Three concrete things come out of that.
...
Every charmarr charm runs a tiny observability sidecar that publishes its own topology metrics. The sidecar accepts a callback hook: any charm can hand it a small piece of code that emits additional metrics scoped to that charm's domain, with no new exporter, no new port, and no extra wiring.
...
The pattern is intentional: when you build a new charm for the stack, you don't have to write an exporter or stand up a new pipeline to surface domain-specific signal. Bolt a callback onto the topology sidecar and crowsnest does the rest.
