...
Every charmarr charm bundles its own Grafana dashboard. But operating a fleet of charms means caring about *cross-cutting* signal too. Is every required relation wired? Is the request pipeline backed up? Is the VPN tunnel healthy *while* the download clients have downloads in flight? Is the storage broker losing consumers?
...
Crowsnest is deployed and integrated automatically by the [Terraform path](setup.md#with-terraform) when the `cos` variable is set. The manual flow is covered under [the Juju CLI walkthrough](setup.md#with-the-juju-cli).
