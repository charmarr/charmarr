...
"""Helper functions for jellyfin-k8s integration tests."""
...
def pack_jellyfin_charm() -> Path:
    """Pack the jellyfin charm and return path to .charm file."""
    logger.info("Packing charm from %s", CHARM_DIR)
    return pack(CHARM_DIR)


def deploy_jellyfin_charm(juju: jubilant.Juju, charm_path: Path) -> None:
    """Deploy jellyfin charm with storage relation."""
    status = juju.status()
