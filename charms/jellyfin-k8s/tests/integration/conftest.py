...
"""Pytest configuration for jellyfin-k8s integration tests."""
...
from tests.integration.helpers import deploy_jellyfin_charm, pack_jellyfin_charm
...
@pytest.fixture(scope="module")
def jellyfin_deployed(juju: jubilant.Juju, charm_path: Path, storage_deployed: None) -> None:
    """Ensure jellyfin is deployed with storage."""
    status = juju.status()
    if "jellyfin" in status.apps:
        return
    deploy_jellyfin_charm(juju, charm_path)
