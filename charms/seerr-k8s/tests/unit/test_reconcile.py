...
"""Unit tests for SeerrCharm reconciliation."""
...
def test_reconcile_non_leader_adds_check_layer(ctx):
    ...
    state = ctx.run(
        ctx.on.pebble_ready(SEERR_CONTAINER),
        State(
            leader=False,
    ...
    container = state.get_container("seerr")
    assert "seerr-check" in container.layers
...
def test_reconcile_adds_pebble_layer(ctx):
    ...
    container = state.get_container("seerr")
    assert "seerr" in container.layers
...
def test_reconcile_sets_log_level(ctx):
    ...
    container = state.get_container("seerr")
    layer = container.layers["seerr"]
    env = layer.services["seerr"].environment
