...
class SeerrCharm(ops.CharmBase):
    """Seerr content request management charm."""
    ...
    def _is_service_running(self) -> bool:
        """Check if Seerr service is running."""
        services = self._container.get_services(SERVICE_NAME)
        return bool(services) and services[SERVICE_NAME].is_running()

    def _get_api_client(self, api_key: str) -> SeerrApi:
        """Create authenticated API client for Seerr."""
        return SeerrApi(f"http://localhost:{WEBUI_PORT}", api_key)

    def _is_workload_ready(self, api_key: str) -> bool:
        """Check if Seerr workload is ready to accept API calls."""
        try:
    ...
    def _build_pebble_layer(self) -> ops.pebble.LayerDict:
        """Build Pebble layer - runs Seerr node process directly (ADR-016)."""
    ...
    def _reconcile_servers(  # pragma: no cover
        ...
        Juju relations are the single source of truth. Servers configured
        manually in Seerr UI that are not backed by a Juju relation will
        be deleted.
        """
    ...
    def _reconcile_media_managers(self, api: SeerrApi) -> None:  # pragma: no cover
    ...
    def _publish_requirer_data(self) -> None:  # pragma: no cover
        ...
        data = MediaManagerRequirerData(
            requester=RequestManager.SEERR,
    ...
    def _configure_ingress(self) -> None:
        ...
        Seerr does not support URL path prefixes - it must be served from
        root. Use a dedicated subdomain (e.g., requests.example.com) via
    ...
    def _reconcile(self, _: ops.EventBase) -> None:
        ...
        if not api_key:
            logger.info("API key not yet available, waiting for Seerr to start")
            return
    ...
    def _on_rotate_api_key_action(self, event: ops.ActionEvent) -> None:
        ...
        if self._is_service_running():
            ...
            logger.info("Restarted Seerr after API key rotation")
    ...
    def _on_import_config_action(self, event: ops.ActionEvent) -> None:
        ...
        try:
            self._container.list_files(path)
        except ops.pebble.Error:
            event.fail(f"Tarball not found at {path}")
            return

        if expected_sha:
            try:
                sha_out, _ = self._container.exec(["sha256sum", path]).wait_output()
            except ops.pebble.ExecError as e:
                event.fail(f"Failed to compute sha256: {e}")
                return
        ...
        event.set_results(
            {"result": f"Config imported from {path}; Seerr will auto-migrate on start"}
        )
