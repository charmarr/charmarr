...
def test_reconcile_builds_pebble_layer(ctx, mock_k8s, tmp_path):
    ...
    layer = container_out.layers.get("radarr")
    assert layer is not None
    service = layer.services["radarr"]
    assert service.user_id == 1000
    assert service.group_id == 1000
    assert "Radarr" in service.command
...
def test_non_leader_only_registers_check(ctx, mock_k8s):
    ...
    container_out = state.get_container("radarr")
    check_layer = container_out.layers.get("radarr-check")
    assert check_layer is not None
    assert "radarr-ready" in check_layer.checks
...
def test_reconcile_skips_write_when_config_matches(ctx, mock_k8s, tmp_path):
    ...
    config_file = config_dir / "config.xml"
    config_content = (
        f"<Config><ApiKey>{TEST_API_KEY}</ApiKey><UrlBase>/radarr-k8s</UrlBase>"
        "<Port>7878</Port><BindAddress>*</BindAddress></Config>"
    )
...
def test_reconcile_syncs_trash_profiles_when_workload_ready(ctx, mock_k8s, tmp_path):
    ...
    with (
        ...
        patch("charm.RadarrCharm._reconcile_download_clients"),
        patch("charm.RadarrCharm._reconcile_root_folder"),
        patch("charm.RadarrCharm._get_quality_profiles", return_value=[]),
        patch("charm.RadarrCharm._get_root_folders", return_value=[]),
    ):
        ctx.run(
            ctx.on.config_changed(),
            State(
...
def test_reconcile_calls_download_client_reconciler(ctx, mock_k8s, tmp_path):
    ...
    with (
        ...
        patch("charm.ensure_pebble_user"),
        patch("charm.reconcile_gateway_client"),
        patch("charm.RadarrCharm._sync_trash_profiles"),
        patch("charm.RadarrCharm._reconcile_download_clients") as mock_reconcile,
        patch("charm.RadarrCharm._reconcile_root_folder"),
        patch("charm.RadarrCharm._get_quality_profiles", return_value=[]),
        patch("charm.RadarrCharm._get_root_folders", return_value=[]),
    ):
...
def test_reconcile_calls_root_folder_reconciler(ctx, mock_k8s, tmp_path):
    ...
    with (
        ...
        patch("charm.RadarrCharm._sync_trash_profiles"),
        patch("charm.RadarrCharm._reconcile_download_clients"),
        patch("charm.RadarrCharm._reconcile_root_folder") as mock_reconcile,
        patch("charm.RadarrCharm._get_quality_profiles", return_value=[]),
        patch("charm.RadarrCharm._get_root_folders", return_value=[]),
    ):
        ctx.run(
