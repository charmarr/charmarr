Feature: Storage charm with hostpath backend

  Background:
    Given the charmarr-multimeter charm is deployed
    And the charmarr-storage charm is deployed with hostpath backend

  Scenario: Storage charm creates PV with hostPath configuration
    Then a PV named "charmarr-shared-media-pv" should exist
    And the PV should have hostPath from config
