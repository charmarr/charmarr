...
from .conftest import SCRAPARR_CONTAINER, SONARR_CONTAINER
...
def _make_traefik_ingress_relation(url: str = "http://10.0.0.1/charmarr-sonarr") -> Relation:
...
def test_url_base_defaults_to_app_name(ctx, mock_k8s, tmp_path):
    ...
    assert "<UrlBase>/sonarr-k8s</UrlBase>" in (config_dir / "config.xml").read_text()
...
def test_url_base_follows_traefik_ingress(ctx, mock_k8s, tmp_path):
    ...
    assert "<UrlBase>/charmarr-sonarr</UrlBase>" in (config_dir / "config.xml").read_text()
...
def test_non_leader_only_registers_check(ctx, mock_k8s):
    ...
    with patch("charm.reconcile_gateway_client"):
        state = ctx.run(
            ctx.on.config_changed(),
            State(leader=False, containers=[SONARR_CONTAINER, SCRAPARR_CONTAINER]),
        )
