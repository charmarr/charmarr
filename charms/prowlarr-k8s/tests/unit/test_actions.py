...
"""Unit tests for ProwlarrCharm actions."""
...
def test_rotate_api_key_action(ctx, mock_k8s, tmp_path):
    ...
    with (
        ...
        patch("charm.ProwlarrCharm._is_service_running", return_value=False),
...
def test_sync_indexers_action(ctx, mock_k8s, tmp_path):
    """Test sync-indexers action calls _reconcile_media_managers."""
    config_file = tmp_path / "config.xml"
    config_file.write_text(CONFIG_XML)
    ...
    with (
        ...
        patch("charm.ensure_pebble_user"),
        patch("charm.reconcile_gateway_client"),
        patch("charm.ProwlarrCharm._reconcile_media_managers") as mock_reconcile,
        patch(
            "charm.ProwlarrCharm._get_api_key_secret",
...
def test_sync_indexers_action_not_leader(ctx, mock_k8s):
    """Test sync-indexers action fails for non-leader."""
    with (
        patch("charm.reconcile_gateway_client"),
        pytest.raises(ActionFailed) as exc_info,
    ):
...
def test_sync_indexers_no_api_key(ctx, mock_k8s, tmp_path):
    """Test sync-indexers action fails when no API key secret exists."""
    config_file = tmp_path / "config.xml"
    config_file.write_text(CONFIG_XML)
    ...
    with (
        patch("charm.ensure_pebble_user"),
        patch("charm.reconcile_gateway_client"),
        patch("charm.ProwlarrCharm._get_api_key_secret", return_value=None),
        pytest.raises(ActionFailed) as exc_info,
    ):
        ctx.run(
            ctx.on.action("sync-indexers"),
