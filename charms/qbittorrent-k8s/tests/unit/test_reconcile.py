...
def test_reconcile_creates_credentials_secret(ctx, mock_k8s):
    """Reconcile creates credentials secret when not exists."""
    ...
    assert "username" in secret.tracked_content
    assert "password" in secret.tracked_content
...
def test_reconcile_builds_pebble_layer(ctx, mock_k8s):
    ...
    container_out = state.get_container("qbittorrent")
    layer = container_out.layers.get("qbittorrent")
...
def test_reconcile_calls_storage_volume(ctx, mock_k8s):
    ...
    with (
        ...
        assert call_kwargs["container_name"] == "qbittorrent"
...
def test_ensure_user_exists_adds_user_and_group(ctx, mock_k8s):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        container = Container(
            name="qbittorrent",
            can_connect=True,
            execs={Exec(["chown", "-R", "1234:5678", "/config/qBittorrent"])},
            mounts={
        ...
        with patch("charm.QBittorrentCharm._is_workload_ready", return_value=False):
            ctx.run(
                ctx.on.config_changed(),
                State(
                    leader=True,
...
def test_secret_rotate_generates_new_credentials(ctx, mock_k8s):
    """Secret rotation generates new password and rewrites config."""
    ...
    secret = Secret(
        tracked_content={"username": "charmarr", "password": "old-password"},
        label="credentials",
        owner="app",
    )
    ...
    with (
        patch("charm.QBittorrentCharm._is_workload_ready", return_value=False),
        patch("charm.QBittorrentCharm._is_service_running", return_value=False),
        patch("charm.ensure_pebble_user"),
    ):
        state = ctx.run(
    ...
    assert rotated_secret.tracked_content["password"] != "old-password"
